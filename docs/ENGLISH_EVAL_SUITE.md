...
- The runner scores each distinct text and scoring prompt once per suite run, then reuses that prediction when calculating statistics for multiple source labels. Different aspect prompts still receive separate scores.
...
| ELLIPSE test | Grammar; Cohesion | G; C | 2,571 | 423.7 words | English-learner state-assessment essays, often argumentative school writing | Intended formal; realized quality varies widely |
...
**Texts.** The final suite uses the official test partition: `n = 2,571` distinct essays (`5,142` eval records across two dimensions); mean 423.7 words, median 393, range 27–1,274. They are US state-assessment essays by English learners in grades 8–12. Much is argumentative school writing addressed to a teacher or principal. The intended register is formal school prose, but proficiency, spelling, punctuation, organization, and length vary widely. The 3,911 official training essays belong exclusively to the external-development panel.
...
**Official sources:** [authors' repository and dataset](https://github.com/scrosseye/ELLIPSE-Corpus), [official rubric](https://github.com/scrosseye/ELLIPSE-Corpus/blob/main/ELL_Rubrics.docx), [authors' preprint](https://zenodo.org/records/11217937), [final-suite test split](../data/benchmarks/ellipse/ELLIPSE_Final_github_test.csv). The separate official train split is used only by the external-development panel.
...
3. **The manifest is tied to the current loader semantics.** It was regenerated on 14 September 2026 and records 1,738 rows for each WebNLG-2017 dimension, 2,469 for each WebNLG-2020 dimension, and 37,983 records across the 31 NLG dimensions. Earlier direct scans differed by one row per affected dimension. Preserve the regenerated manifest with results and do not compare counts from different loader revisions as though they were the same evaluation population.
...
- Standalone dataset files: [ELLIPSE test](../data/benchmarks/ellipse/ELLIPSE_Final_github_test.csv), [Human–ChatGPT argumentative essays](../data/benchmarks/human-chatgpt-argumentative-essays.csv), [CoheSentia test](../data/benchmarks/CohesentiaTestData.json)
