# This script has been co-created, refactored, and cleaned with GPT 5.6.
...
def flatten_regression_split(dataset: Dataset, score_name: str, split_name: str, *, exclude_layer_zero: bool = False) -> tuple[Dataset, dict[str, int]]:
    if score_name not in dataset.column_names:
        raise ValueError(f"Split {split_name!r} does not contain score field {score_name!r}.")
    rows, missing, invalid, excluded_layer_zero = [], 0, 0, 0
    for chain in dataset:
        ...
        for index, (text, layer, score) in enumerate(zip(fields["texts"], fields["labels"], fields["scores"])):
            if exclude_layer_zero and int(layer) == 0:
                excluded_layer_zero += 1
                continue
            if score is None:
                missing += 1; continue
    ...
    stats = {"accepted": len(rows), "skipped_missing": missing, "skipped_invalid": invalid,
             "excluded_layer_zero": excluded_layer_zero}
    if not rows:
