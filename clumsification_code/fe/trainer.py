...
from .evaluation import evaluate_pairwise_accuracy_distributed
from .utils import tensor_debug_summary
from .losses import canonicalize_loss_name


class PairwiseFETrainer(Trainer):
    def __init__(
        self,
        epsilon: float = 0.2,
        scale: float = 5.0,
        ...
        pairwise_eval_tokenizer=None,
        pairwise_eval_max_length: Optional[int] = None,
        ...
        **kwargs,
        ...
        self.pairwise_eval_tokenizer = pairwise_eval_tokenizer
        self.pairwise_eval_max_length = pairwise_eval_max_length
    ...
    def evaluate(
        ...
        if self.pairwise_eval_tokenizer is None or self.pairwise_eval_max_length is None:
            raise ValueError(
                "pairwise_eval_tokenizer and pairwise_eval_max_length are required for evaluation."
            )

        metrics = evaluate_pairwise_accuracy_distributed(
            model=self.model,
            dataset=eval_dataset,
            tokenizer=self.pairwise_eval_tokenizer,
            max_length=self.pairwise_eval_max_length,
