...
def generateBatchItem(og_dataset_name, batch_id, text, model_name, effort, text_format, language, perturbation_type):
...
def readBatchResponseContents(lines, model_type, perturbation_type):
    completions = []
...
def main(cmd_args):
    ...
    if not os.path.exists(custom_ds_folder+str(perturbation_layer+1)+".jsonl"):
        ...
        with open(custom_ds_folder+input_name, 'r') as reader:
            for l in reader:
                ...
                batch_items.append(generateBatchItem(input_name, og_id, og_text, model_name, effort, response_format, language, perturbation_type))
    ...
    if batch_job.error_file_id:
        ...
        if batch_job.output_file_id:
            ...
            completions = readBatchResponseContents(file_response.read().splitlines(), model_name, perturbation_type)
